...
DIAS = int(os.getenv("DIAS_RECEBIMENTOS", "30"))
...
# =========================
# TRATAMENTO RECEBIMENTOS
# =========================
def tratar_recebimentos(lista, token: str, data_inicio_ref: str):
    resultado = []
...
def main():
    inicio = data_inicio()
    log(f"🚀 INICIOU SYNC RECEBIMENTOS | últimos {DIAS} dias | desde {inicio}")

    token = get_token()

    recebimentos = get_data_paginado("recebimentos", token)
    recebimentos_tratados = tratar_recebimentos(recebimentos, token, inicio)

    log(f"qtd recebimentos tratados: {len(recebimentos_tratados)}")

    deletar_por_data("eg_recebimentos", "data", inicio)
    enviar_supabase("eg_recebimentos", recebimentos_tratados)

    log("✅ FINALIZADO SYNC RECEBIMENTOS")
