...
def tratar_pagamentos(lista, token: str):
    ...
    for idx, item in enumerate(lista, start=1):
        ...
        if idx % 200 == 0:
            log(f"🔄 pagamentos processados: {idx}/{total}")
...
def main():
    log("🚀 INICIOU SYNC PAGAMENTOS FULL")
    ...
    limpar_tabela_total("eg_pagamentos")
    upsert_lote("eg_pagamentos", pagamentos_tratados, on_conflict="id_origem")

    log("✅ FINALIZADO SYNC PAGAMENTOS FULL")
